...
The package bundles the canonical `SCHEMA/1.0` tree from this repository and preloads those schemas into AJV, so
consumers validate against the same artifacts used by the protocol release.
...
- `validateRequest(message)` validates a request against the envelope and, when available, the matching `*.request`
  method schema.
...
```javascript
const { validateRequest, validateWithSchema, schemas } = require("gabp-schemas");
...
```typescript
import { validateRequest, type GabpMessage, type ValidationResult } from "gabp-schemas";
